...
export class RealtimeService {
  ...
  public unconnectedProjects: Array<DataSource> = [];
  constructor(
  ...
  /**
   * Whenever for any sort of reason if the project is changed, either by user details changes
   * api call for value change, and a cron job, all instances of the the dashboard will recieve this function
   */
  public RecieveProjectChange (project: any) {
    this.store.dispatch({
      type: 'UPDATE_PROJECT',
      payload: project
    });
  ...
  public RecieveDataSourceIncoming (data: DataSource) {
    ...
    const projectWithThisSource = this.projects.find(x => x.datasource === data.dataSourceId);
  ...
  public MockDataIncome () {
    const data: DataSource = {
      ...
      dataSourceId: 'project-' + random(1, 2)
    };
    this.RecieveDataSourceIncoming(data);
  }
   ...
   */
  public ChangeComplexOutputValue (projectDataSourceId, pin, value) {
    this.store.dispatch({
      type: 'UPDATE_COMPLEX_PROJECT',
      payload: {
        project: projectDataSourceId,
        pin,
        value
      }
    });
    this.requests.changeComplexBoardOutput(projectDataSourceId, pin, value).then((response) => {
    }). catch(error => {
