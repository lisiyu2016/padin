...
export class LocationRowComponent implements OnInit {
  ...
  @Input() public widgets: Array<CloudProject> = [];
  ...
  /**
   * In case that in location single we have set a project as primary temperature for this location,
   * we will display it here.
   */
  public findTemperature (location: ILocation) {
    this.store.select('iotModule').subscribe(({projects}) => {
      /* tslint:disable */
      const project = projects.find(d => d.id == location.temperatureProject);
      if (project) {
        this.primaryTemperature = project.value;
      }
    });
  }
  setPrecent(precent) {
